...
export default function RegisterPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError(msg.includes('email-already-in-use') ? 'An account with this email already exists.' : 'Could not create account. Please try again.');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-teal-50 to-blue-50 flex items-center justify-center px-4 py-20">
      <div className="w-full max-w-md">
        <div className="bg-white rounded-2xl shadow-xl p-8 border border-gray-100">
          <div className="text-center mb-8">
            <Link href="/" className="inline-flex items-center gap-1 mb-6">
              <span className="text-teal-600 font-black text-2xl">+</span>
              <span className="text-gray-900 font-bold text-xl">plus7<span className="text-teal-600">hälsa</span></span>
            </Link>
            <h1 className="text-2xl font-black text-gray-900">Create your account</h1>
            <p className="text-gray-500 text-sm mt-1">Join for free and access exclusive resources</p>
          </div>

          {/* Benefits */}
          <div className="bg-teal-50 rounded-xl p-4 mb-6 space-y-2">
            {['Access to resource library', 'Register for webinars', 'Receive health insights newsletter'].map((b) => (
              <div key={b} className="flex items-center gap-2 text-sm text-teal-700">
